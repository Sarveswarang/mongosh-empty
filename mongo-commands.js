...
db.movies.find({rating:{$lt:8.5}},{trailer:0,rating:0}).pretty()
// exclusion for id and inclusion
db.movies.find({rating:{$lt:8.5}},{_id:0,name:1,rating:1}).pretty()
// lover than or equal to
db.movies.find({rating:{$lte:8}},{_id:0,name:1,rating:1}).pretty()
db.movies.find({rating:{$in:[8,8.8]}},{_id:0})
db.movies.find({rating:{$nin:[8,8.8]}},{_id:0})
db.movies.find({rating:{$gt:8.5,$lt:9}},{_id:0,name:1,rating:1}).pretty()
...
db.orders.find({sum:{quantity}})
db.orders.find({},{_id:0,status:"urgent"}.sum({status:"urgent"}))
db.orders.find({},{_id:0,status:"urgent"})
select * from orders;
where status="urgent";
db.orders.aggregate([{$match:{status:"urgent"}},
{$group :{_id:"$productName",total:{$sum:"$quantity"}}}]).pretty()
